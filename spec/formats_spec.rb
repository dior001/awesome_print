...
RSpec.describe 'AwesomePrint' do
    ...
    if RUBY_VERSION > '1.9'
      ...
      it 'plain single line' do
        expect(@set.sort_by(&:to_s).ai(plain: true,
                                       multiline: false)).to eq(@arr.sort_by(&:to_s).ai(
                                                                  plain: true, multiline: false
                                                                ))
      end
  ...
  describe 'Struct' do
    ...
    it 'handles a self-referential struct without infinite recursion' do
      nested_struct = Struct.new('NestedStruct', :value).new
      nested_struct.value = nested_struct
      expect(nested_struct.ai(plain: true)).to include('value = {...}')
    end
